...
def conviction_size(
    *,
    ...
    win probability, the leash, and ``kelly_fraction``. Then clamped to the
    per-division/per-trade cap, the deployable headroom, and (for Event) the
    hard cap. Returns 0.0 if the edge is non-positive.
    """
    if edge <= 0 or risk_unit_fraction <= 0 or win_probability <= 0:
        return 0.0

    bankroll = caps.total_deployable_cad if bankroll_cad is None else bankroll_cad

    # Fraction of bankroll to risk, before caps.
    ...
    # Hard caps the CEO cannot override.
    per_trade = caps.cap_for(division.value)
    size = min(size, per_trade)
